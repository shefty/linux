...
#define CM_DIRECT_RETRY_CTX ((void *) (unsigned long) 1)
...
cm_alloc_response_msg_no_ah(struct cm_port *port,
			    struct ib_mad_recv_wc *mad_recv_wc, void *ctx)
{
	struct ib_mad_send_buf *m;
				 ...
				 struct ib_mad_send_buf **msg)
{
	...
	m = cm_alloc_response_msg_no_ah(port, mad_recv_wc, ctx);
	if (IS_ERR(m))
		return PTR_ERR(m);
			...
			void *ari, u8 ari_length)
{
	...
	ret = cm_alloc_response_msg(port, mad_recv_wc, NULL, &msg);
	if (ret)
		return ret;
			       ...
			       struct cm_id_private *cm_id_priv)
{
	...
	ret = cm_alloc_response_msg(work->port, work->mad_recv_wc,
				    CM_DIRECT_RETRY_CTX, &msg);
	if (ret)
		return;
			 ...
			 struct ib_mad_recv_wc *mad_recv_wc)
{
	...
	ret = cm_alloc_response_msg(port, mad_recv_wc, CM_DIRECT_RETRY_CTX, &msg);
	if (ret)
		return ret;
